...
let describe character =
  character.name ^ " was " ^ describe_location character.location
...
let tell_context context =
  let rec aux = function 
    ...
    | character :: characters ->
      describe character ^ aux characters
  in
...
let possible_changes_for_characer character context =
  match character with 
  | { state = Happy } -> [Hungry]
  | { state = Hungry } -> []
;;
...
let possible_changes context = 
  let rec aux = function
    | [] -> []
    | character :: cs ->
      let possible_changes = possible_changes_for_characer character context in
      let rec aux' = function
        | [] -> []
